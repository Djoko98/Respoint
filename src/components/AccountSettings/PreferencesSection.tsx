...
const PreferencesSection: React.FC<PreferencesSectionProps> = memo(({
  formData,
  onInputChange
}) => {
  const { t } = useLanguage();
  return (
    <div className="bg-[#0A1929] border border-gray-800 rounded p-6">
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-xs text-gray-500 mb-2">{t('applicationLanguage')}</label>
          <select
        ...
        <div className="flex items-center justify-between">
          <div>
            <h4 className="text-sm text-white">{t('autoArchiveReservations')}</h4>
            <p className="text-xs text-gray-500 mt-1">
              {t('autoArchiveDescription')}
            </p>
          </div>
          <button
            onClick={() => onInputChange('autoArchive', !formData.autoArchive)}
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
              formData.autoArchive ? 'bg-blue-600' : 'bg-gray-700'
            }`}
          >
            <span
              className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                formData.autoArchive ? 'translate-x-6' : 'translate-x-1'
              }`}
            />
          </button>
        </div>
